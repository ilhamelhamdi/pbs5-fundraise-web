...
export default class Header extends React.Component {
    ...
    handleScroll = () => {
        ...
        const isTop = ["bg-gradient-to-b", "from-black", "bg-opacity-20"]
    ...
    render() {
        return (
            <header className="w-screen h-14 z-10 text-white fixed transition duration-300 ease-in-out bg-gradient-to-b from-black bg-opacity-20">
                <div className="container mx-auto w-4/5 h-full flex flex-row justify-between items-center">
                    ...
                    <nav className="flex w-max items-center">
                        {navigasi.map((e, index) => <Link href={e.link} key={index}><a className="nav-header inline-block rounded-lg  mx-2 p-2 text-l bg-opacity-0 bg-white hover:bg-opacity-10 transition-all">{e.name}</a></Link>)}
                    </nav>
                    <div className="flex w-max items-center">
                        {user.map((e, index) => <Link href={e.link} key={index}><a className="nav-header inline-block rounded-lg  mx-2 p-2 text-l bg-opacity-0 bg-white hover:bg-opacity-10 transition-all">{e.name}</a></Link>)}
                    </div>
                </div>
            </header>
        )
